...
class QueMail(Thread):
    ...
    def run(self):
        while not self._do_quit:
            if not self._queue.empty():
                log.debug("Connecting to SMTP server: %s:%i" % (self.smtp_host, self.smtp_port))
                smtp = None
                try:
                    smtp = smtplib.SMTP()
                    ...
                    while not self._queue.empty():
                        t = time.time()
                        eml = self._queue.get()
                        log.info("Sending (qs=%i): %s" % (self._queue.qsize(), eml))
                        try:
                            msg = eml.as_rfc_message()
                            content = msg.as_string()
                            log.debug("with content: %s" % content)
                            smtp.sendmail(eml.adr_from, eml.adr_to, content) 
                            log.warning("Sent (qs=%i,t=%f): %s" % (self._queue.qsize(),time.time()-t, eml))
                        except Exception as e:
                            log.error("Exception occured while sending email: %s" % eml)
                            log.exception(e)
    ...
    def send(self, eml):
        ...
        log.debug('Accepted (qs=%i): %s' % (self._queue.qsize(), eml))
