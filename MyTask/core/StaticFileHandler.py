    ...
    **Subclassing notes**
    ...
    def make_static_url(cls, settings, path, include_version=True):
        ...
        if environment == 'development':
            if file_type == 'css/':
                origin_path = settings.get('assets_css_prefix', 'less/') + path[4:-4] + '.less'
                origin_abs_path = cls.get_absolute_path(settings['assets_path'], origin_path)
                
                origin_stat_result = os.stat(origin_abs_path )
                origin_modified = datetime.datetime.utcfromtimestamp(origin_stat_result[stat.ST_MTIME])
                
                abs_path = cls.get_absolute_path(settings['static_path'], path)
                stat_result = os.stat(abs_path)
                dest_modified = datetime.datetime.utcfromtimestamp(stat_result[stat.ST_MTIME])
                
                if origin_modified > dest_modified or path == 'css/application.css':
                    command_opt = ['lessc']
